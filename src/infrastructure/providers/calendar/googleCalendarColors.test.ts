...
describe('googleCalendarColors', () => {
  it('maps planner priorities to Google event colorIds', () => {
    ...
    expect(googleEventColorIdFromPriority('P4')).toBe('8');
    expect(googleEventColorIdFromPriority('DROP')).toBe('8');
  });
  ...
  it('maps known Personal OS hex colors', () => {
    ...
    expect(googleEventColorIdFromHex('#64748b')).toBe('8');
    expect(googleEventColorIdFromHex('#ca8a04')).toBe('8');
    expect(googleEventColorIdFromHex('#705CF6')).toBe('9');
  });
