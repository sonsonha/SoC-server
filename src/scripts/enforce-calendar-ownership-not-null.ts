...
import { calendarCommitments, integrationTokens } from '../infrastructure/db/schema/index.js';
...
async function main() {
  ...
  // Resolve from cwd so this works from dist/scripts in the Railway image
  // (drizzle/ is copied next to package.json, not next to dist/).
  const sqlPath = path.resolve(process.cwd(), 'drizzle/0020_google_calendar_ownership_not_null.sql');
