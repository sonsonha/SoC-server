  ...
  async getGoalProgress(userId: string, id: string, nowIso?: string) {
    ...
    const progress = buildGoalProgress(
      ...
      blockRows.map((block) => ({
        ...
        status: block.status,
        outcomeType: (block.sessionOutcomeType as 'NONE' | 'CHECKLIST' | 'QUANTITY' | null) ?? 'NONE',
        outcomeTarget: block.sessionOutcomeTarget,
        outcomeActual: block.sessionOutcomeActual,
      })),
